...
goto comment
    ...
    This batch file contains commands that run the BasicAnalyzeNVTest.java sample with different arguments.
      ...
      * The third command, which is commented out, displays the list of supported arguments for the BasicAnalyzeNVTest.java sample.

    BasicAnalyzeNVTest.java description:
...
java -cp ../../target/hpe-nv-java-samples-1.0.0.jar com.hpe.nv.samples.basic.BasicAnalyzeNVTest --server-ip 0.0.0.0 --server-port 8182 --username MyNVUser --password MyNVPassword --proxy MyProxyHost:MyProxyPort --zip-result-file-path analysis-report-basic.zip

REM Run the sample with the analysis result .zip file argument and debug messages
REM java -cp ../../target/hpe-nv-java-samples-1.0.0.jar com.hpe.nv.samples.basic.BasicAnalyzeNVTest --server-ip 0.0.0.0 --server-port 8182 --username MyNVUser --password MyNVPassword --proxy MyProxyHost:MyProxyPort --zip-result-file-path analysis-report-basic.zip --debug true

REM Run the sample with the --help flag to get the list of supported arguments
REM java -cp ../../target/hpe-nv-java-samples-1.0.0.jar com.hpe.nv.samples.basic.BasicAnalyzeNVTest --help
